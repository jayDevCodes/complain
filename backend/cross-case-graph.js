...
function caseFiles(caseDir) {
  ...
  return fs.readdirSync(caseDir)
    .filter(name => name.endsWith('.json') && !name.endsWith('.graph.json'))
...
function buildCrossCaseGraph(caseDir) {
  ...
  const addNode = node => {
    if (!node || graph.nodes.some(x => x.id === node.id)) return;
  ...
  for (const c of cases) {
    const caseNode = entity('CASE', c.investigationId || c.title || 'case', c.title || c.investigationId || 'Investigation Case', { investigationId: c.investigationId });
    addNode(caseNode);
    const ids = new Set();
    ...
    caseEntityLists.push({ case: c, node: caseNode, ids });
  }

  for (let i = 0; i < caseEntityLists.length; i++) {
    for (let j = i + 1; j < caseEntityLists.length; j++) {
      const a = caseEntityLists[i];
      const b = caseEntityLists[j];
      ...
      const relation = edge(a.node.id, 'RELATED_CASE', b.node.id, similarity, shared.slice(0, 30));
      addEdge(relation);
