...
function compactCase(caseData) {
  ...
  const lead = caseData.title || 'Government work investigation';
...
function objectTerms(caseData) {
  ...
  const text = candidates.map(x => {
    const r = x.result || x;
    return r.object || r.description || r.category || '';
  }).filter(Boolean).join(' ');
  const tenderText = JSON.stringify(caseData.tender || {});
  const comparisonText = JSON.stringify(caseData.comparison || []).slice(0, 2500);
  return [text, tenderText, comparisonText].join(' ').replace(/\s+/g, ' ').slice(0, 3500);
}
...
function buildComparativeQueries(caseData) {
  ...
  return [
    ...
    { type: 'INSPECTION_FAILURES', query: `${place} ${object} inspection report measurement book completion certificate defect notice`, rationale: 'Search for examples of execution-stage failures and official responses.' },
...
function normalizeSource(result, queryMeta) {
  ...
  return source(result.url, result.title || '', host, 'comparative-web', result.excerpt || result.content || '')
    ? { ...source(result.url, result.title || '', host, 'comparative-web', result.excerpt || result.content || ''), queryType: queryMeta.type, rationale: queryMeta.rationale }
    : null;
}
...
function dedupeSources(rows) {
  ...
  for (const row of rows.filter(Boolean)) {
    ...
    if (!map.has(key)) map.set(key, row);
      ...
      old.queryTypes = [...new Set([old.queryType, old.queryTypes || [], row.queryType].flat())].filter(Boolean);
...
async function runComparativeResearch(caseData) {
  ...
  const patterns = usable.flatMap(x => x.patterns || x.similarCases || x.recurringPatterns || []).slice(0, 40);
  const priorComplaints = usable.flatMap(x => x.priorComplaints || x.complaints || []).slice(0, 40);
  const outcomes = usable.flatMap(x => x.outcomes || x.priorOutcomes || []).slice(0, 40);
  const loopholes = usable.flatMap(x => x.loopholes || x.controlGaps || x.riskPatterns || []).slice(0, 40);
  const contradictions = usable.flatMap(x => x.contradictions || x.disagreements || []).slice(0, 30);
  ...
  return {
    ...
    findings: { patterns, priorComplaints, outcomes, loopholes, contradictions },
    methodology: {
...
module.exports = { buildComparativeQueries, runComparativeResearch, compactCase };
